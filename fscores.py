...
scr_list = Person.objects.filter(village__block__district__district_name='Guna').annotate(adoptions=Count('personadoptpractice')).filter(adoptions__gte=5).values_list('id','personmeetingattendance__screening__videoes_screened','personmeetingattendance__screening__date')
scr_date = {}
...
adopter_ids =  Person.objects.filter(village__block__district__district_name='Guna').annotate(adoptions=Count('personadoptpractice')).filter(adoptions__gte=5).values_list('id', flat=True)
adoption_list = []
...
for video in adoption_date.keys():
    video_count[video] = {}
    for village in Village.objects.filter(block__district__district_name='Guna'):
...
for video in adoption_date.keys():
    ...
    for block in Block.objects.filter(district__district_name='Guna'):
...
village_dist = {}  
for v1 in Village.objects.filter(block__district__district_name='Guna'):
    village_dist[v1.id] = {}
    for v2 in Village.objects.filter(block__district__district_name='Guna'):
        if v1 == v2:
